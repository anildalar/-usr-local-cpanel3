/*
# cpanel - whostmgr/docroot/templates/menu/main.js Copyright 2024 cPanel, L.L.C.
#                                                           All rights reserved.
...
(function() {
    ...
    (function() {
        ...
        function dismissImportantNextSteps() {
            ...
            if (window["mixpanel"]) {
                window["mixpanel"].track("WHM-Important-Next-Steps-Dismiss-Clicked");
            }
        ...
        function init() {
            ...
            // Track links in Important Next Steps
            window.addEventListener("load", function() {

                // Ensure the section is present to avoid a warning, since it's not guaranteed to be present
                if (!document.querySelector("#important_next_steps")) {
                    return;
                }
                if (window["mixpanel"]) {
                    window["mixpanel"].track_links("#important_next_steps a", "WHM-Important-Next-Steps-Nav-Link", (linkEl) => {
                        var linkTitle = "";
                        const titleSelect = linkEl.querySelector(".cp-app__details-title");
                        if (titleSelect) {
                            linkTitle = titleSelect.innerHTML;
                        }
                        return { "nav-link-id": linkTitle };
                    });
                }
            });
        }
