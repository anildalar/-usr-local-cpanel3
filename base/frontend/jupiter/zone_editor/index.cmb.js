            ...
            "ManageZoneRecordsController", [
                ...
                function(
                        ...
                        if (record.is_new === "0") {
                            switch (record.record_type) {
                                ...
                                case "AFSDB":
                                    ...
                                    break;
                                case "ALIAS":
                                    newRecord.alias = Zones.trimTrailingDot(record.record);
                                    newRecord.record = Zones.trimTrailingDot(record.record);
                                    break;
                                case "CAA":
                        ...
                        var submitObjs = records.map(function(record) {
                            ...
                            switch (record.record_type) {
                                ...
                                case "AFSDB":
                                    ...
                                    break;
                                case "ALIAS":
                                    parsedRecord.data.push(record.alias);
                                    break;
                                case "CAA":
                        ...
                        return Zones.fetch(manage.domain)
                            .then(function(data) {
                                ...
                                for (var i = 0, len = recordData.length; i < len; i++) {
                                    ...
                                    if (zoneRecord.record_type === "ALIAS") {
                                        if (!PAGE.is_alias_available) {
                                            zoneRecord.isEditable = false;
                                            zoneRecord.alias_unsupported = true;
                                        }
                                    }
                                }
                                filterListFilter();
                    ...
                    manage.filterRowRecordsDisplay = function(record) {
                        ...
                        if (!PAGE.is_alias_available) {
                            disallowedTypes.push("ALIAS");
                        }
                        return (disallowedTypes.indexOf(record.type) === -1);
                    };

                    manage.filterAddRecordDisplay = function(record) {
                        var disallowedTypes = ["SOA"];
                        if (!PAGE.is_alias_available) {
                            disallowedTypes.push("ALIAS");
                        }
                        return (disallowedTypes.indexOf(record.type) === -1);
...
/*
# zone_editor/services/domains.js                 Copyright(c) 2020 cPanel, L.L.C.
#                                                           All rights reserved.
# copyright@cpanel.net                                         http://cpanel.net
# This code is subject to the cPanel license. Unauthorized copying is prohibited
*/
