...
#### IF THIS CHANGES ALSO UPDATE Cpanel::API::DNS::is_alias_available() TO MATCH! ##
sub is_alias_available ( $args, $metadata, $api_info_hr ) {    # third arg seems wonky … but … errors
    $metadata->set_ok();
    ...
    if ( !-e "/etc/pdns/danger-danger-danger-server-owners-understand-the-severe-risk-and-accept-responsibility-for-any-consequences-of-using-experimental-alias-records" ) {
        return { alias => 0, aname => 0, resolver => undef };
    }
    ...
    my %pdns = map { chomp; split /=/, $_ } `/usr/bin/pdns_control current-config | grep -E "^expand-alias|^resolver"`;
    ...
    if ( $expand && $resolver ) {
        return { alias => 1, aname => 0, resolver => $pdns{resolver} };
    }

    return { alias => 0, aname => 0, resolver => $pdns{resolver} };
}
