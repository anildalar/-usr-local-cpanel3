...
sub _product_information {
    ...
    try {
        ...
        $version = $Cpanel::Version::Tiny::VERSION_BUILD // $Cpanel::Version::Tiny::VERSION_BUILD;    # avoid a warning once
    };
    return $version;
}
...
sub system_needs_reboot ( $args, $metadata, @ ) {
    ...
    _try(
        sub {
            Cpanel::LoadModule::load_perl_module('Cpanel::CloudLinux');

            # check if the cpanel license contains CloudLinux
            return unless Cpanel::CloudLinux::licensed();

            # licensed but installation requested to disable it
            return if Cpanel::CloudLinux::nocloudlinux();

            # check if the kernel has cloudlinux support
            return if Cpanel::CloudLinux::kernel_support();

            $details{needs_cloudlinux} = 1;
