...
sub restricted_restore ($self) {
    ...
    my $remote_domains_hr = Cpanel::Config::LoadConfig::loadConfig( $Cpanel::ConfigFiles::REMOTEDOMAINS_FILE, undef, "" );
    if ( defined( $remote_domains_hr->{$main_domain} ) && $remote_domains_hr->{$main_domain} == 1 ) {
        $self->out( locale()->maketext( "The system will not change the MX record, as the primary domain ([_1]) uses an MX record that points to a remote mail server.", $main_domain ) );
        return 1;
    }

    my @domains  = ( $main_domain, @{ $utils->get_original_domains() } );
