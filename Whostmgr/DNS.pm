...
my %record_sanitizer_for_type = (
    ...
    'ALIAS' => sub {
        ...
        if ( !_is_valid_dns_domain( $record->{'alias'} ) ) {
            return 0, 'Supplied ALIAS is invalid.';
        }
        if ( Cpanel::Validate::IP::is_valid_ipv6( $record->{'alias'} ) ) {
            return 0, 'ALIAS can not point to an IPv6 address';
        }
        if ( Cpanel::Validate::IP::v4::is_valid_ipv4( $record->{'alias'} ) ) {
            return 0, 'ALIAS can not point to an IPv4 address';
        }

        # Make sure we have at least two labels.
        if ( $record->{'alias'} !~ /.+\..+\.?\z/ ) {
            return 0, 'ALIAS must point a FQDN (Fully Qualified Domain Name).';
        }

        _keep_only_relevant_keys( $record, 'alias' );
        return 1;
    },
);
...
sub _base_record_is_valid {
    ...
    if ( $record->{'type'} !~ m/\A(?:A|AAAA|CAA|CNAME|MX|PTR|SRV|TXT|NS|ALIAS)\z/ ) {
        push @issues, 'Unsupported record type provided.';
    }

    if ( $record->{'type'} =~ m/ALIAS/ ) {
        require Whostmgr::API::1::DNS;
        my $res = Whostmgr::API::1::DNS::is_alias_available( {}, bless( {}, 'Whostmgr::API::1::Utils::Metadata' ), undef );

        if ( !$res->{alias} ) {
            push @issues, 'ALIAS records are not currently enabled.';
        }
    }
