...
This is the file we actually edit to make configuration changes.
Newer versions of sshd will have an include directive early in the
sshd_config file. Since sshd will use the first value it finds for
a particular setting, options set in one of these include files
will override what is set later in sshd_config.
So, if the sshd_config is using an include directory, then
file_to_edit will be an include file in that directory.
If not, then file_to_edit will be the same as filename.
...
sub file_to_edit {
    ...
    my $includes_hr = $self->_get_include_directives();

    foreach my $value (@$includes_hr) {
        if ( $value =~ m|^(.*/sshd_config.d)/\*| ) {
            $retval = "$1/000-cpanel-options.conf";
            last;
        }
    }
...
sub _get_include_directives {
    my $self = shift;

    if ( defined $self->{'_include_directives'} ) {
        return $self->{'_include_directives'};
    }

    my @include_directives;

    foreach my $line ( split( /\n/, $self->get_raw_conf() ) ) {
        next unless $line =~ m/^\s*Include/i;

        $line =~ s/^\s*//;    # strip leading spaces
        my $value = ( split /(?:\s*=\s*|\s+)/, $line )[1];

        push @include_directives, $value if $value;
    }

    $self->{'_include_directives'} = \@include_directives;
...
sub get_raw_conf {
    my $self = shift;
    return ${ $self->_open_ro( $self->filename() )->get_data() };
}
...
sub set_config {
    ...
    $self->{_last_conf_contents} = ${ $self->_transaction()->get_data() };
...
sub _get_sshd_conf_from_sshd {
    my $self   = shift;
    my $output = Cpanel::SafeRun::Errors::saferunnoerror( $self->_sshd_binary, '-T', '-f', $self->filename() );

    if ( $? != 0 ) {
        die Cpanel::Exception::create( 'Services::NotConfigured', [ service => 'sshd', reason => "The sshd_config file has errors" ] );
    }
...
sub _sshd_binary {
    ...
    unless ( defined $self->{_sshd_binary} ) {
        $self->{_sshd_binary} = Cpanel::Binaries::path('sshd');
    }
...
sub _transaction {
    ...
    return $self->_open_rw( $self->file_to_edit() );
}

sub _open_ro {
    my ( $self, $file ) = @_;
    return $self->_open_transaction_object( 'Cpanel::Transaction::File::RawReader', $file );
}

sub _open_rw {
    my ( $self, $file ) = @_;
    $self->{transaction} = $self->_open_transaction_object( 'Cpanel::Transaction::File::Raw', $file );
...
sub _open_transaction_object {
    ...
    try {
        $transaction = "$module"->new( 'path' => $file, 'permissions' => $permissions, 'restore_original_permissions' => 1 );
    }
    catch {
        my $ex          = $_;
