...
sub is_supported () {
    ...
    return Cpanel::Server::Type::Profile::Roles::is_role_enabled('LiteSpeed');
}

sub install_litespeed {
    my ( $self, $args ) = @_;

    return unless is_supported();
...
Check if LiteSpeed can be used for the product / license.
Returns a boolean. True when LiteSpeed is supported.
