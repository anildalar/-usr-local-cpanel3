...
sub is_supported ($self) {
    ...
    return Cpanel::Server::Type::Profile::Roles::is_role_enabled('JetBackup');
}

sub install_jetbackup {
    my ( $self, $args ) = @_;

    return unless $self->is_supported();
...
Check if JetBackup can be use for the product / license.
Returns a boolean. True when JetBackup is supported.
