...
use POSIX qw(:sys_wait_h WIFEXITED WIFSIGNALED WIFSTOPPED WEXITSTATUS
             WTERMSIG WSTOPSIG);
use if ${^TAINT}, 'Taint::Util', 'untaint';
...
sub untaint_var {
  if(!${^TAINT}) {
    return $_[0] if !wantarray; # If we're not taint and the caller hasn't done a wantarray stunt, then just give back what was passed.
...
*uri_list_canonify = *uri_list_canonify = \&uri_list_canonicalize;  # compatibility alias
