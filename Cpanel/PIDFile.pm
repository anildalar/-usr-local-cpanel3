...
=head2 do_exclusive($class, $pid_file, $todo_cr)

Executes $todo_cr in an exclusive manner after locking $pid_file

=over 2

=item C<SCALAR> Class to use

=item C<SCALAR> Name of pid file to use

=item C<CODEREF> Code to execute

=back

=cut

sub do_exclusive ( $class, $pid_file, $todo_cr ) {
    my $lock = Cpanel::SafeFile::safelock($pid_file);

    $class->do( $pid_file, $todo_cr );
