...
sub get_lock_for_cmd ( $self, $logger, $args ) {
    ...
    if ( $ENV{'CPANEL_RPM_LOCKED_IN_PARENT'} ) {
        $logger && $logger->info("Ignoring cmd lock owned by parent process $ENV{CPANEL_RPM_LOCKED_IN_PARENT}");
        return $self->wait_for_locks($logger);
    }

    my $lock = $self->hold_lock( $logger, $args );
    $self->wait_for_locks($logger);

    $ENV{'CPANEL_RPM_LOCKED_IN_PARENT'} = $$;

    my $on_release = Cpanel::Finally->new( sub { delete $ENV{'CPANEL_RPM_LOCKED_IN_PARENT'} } );

    return [ $lock, $on_release ];
}
