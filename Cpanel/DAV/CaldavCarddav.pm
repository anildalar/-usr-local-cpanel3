...
sub _get {
    ...
    if ( -f $path && -r _ ) {
        ...
        if ( $self->check_read_access( $request->uri->path ) ) {
            ...
            if ( -l $path && $path =~ /\.ics/ && ( my $customized_ics_ref = $self->_load_custom_transp_or_valarm( $path, \$content ) ) ) {
                dbg("=[_get]= : applying customized transp and/or valarm for $path");
                $content = $$customized_ics_ref;
            }
            else {
                dbg("=[_get]= : no need to apply custom transp and/or valarm for $path");
            }
...
sub _post {    ##no critic(Subroutines::ProhibitExcessComplexity)
    ...
    my $path = $self->{'request_info'}{'fs_root'};
    dbg("=[_post]= : path = ([$path])");
    if ( _is_path_too_long($path) ) {
        $response->code('413');
        $response->message("File name too long");
        goto QUICKEXIT;
    }
    ...
    if ( !length( $headers_obj->header('Transfer-Encoding') ) || lc( $headers_obj->header('Transfer-Encoding') ) ne 'chunked' ) {
        ...
        my $data_path = $path;
        dbg("=[_post]= : data_path at this point is ([$data_path])");    # usually the path to the .ics

        if ( $post_dispo_type eq 'attachment' ) {
            dbg("=[_post]= : dealing with an attachment related to ([$data_path])");

            if ( $data_path =~ m/^(.*)\.ics$/ ) {
                $attach_dir = $1 . '.cpd';
                if ( !mkdir( $attach_dir, 0700 ) ) {
                    dbg("=[_post]= : Could not mkdir ([$attach_dir]) : $!");
                }
            }
            require Cpanel::Rand::Get;
            require Cpanel::DAV::UUID;
            $managed_id      = Cpanel::Rand::Get::getranddata(20);
            $attachment_uuid = Cpanel::DAV::UUID::generate();

            # Can save the original name in $data_path\.name or something if we decide it is needed. Currently the only source of this information is in the .ics ATTACH line.
            # require MIME::Base64;
            # my $attachment_name_encoded = MIME::Base64::encode_base64( $filename, '' );

            dbg("=[_post]= : managed id is $managed_id");
            $data_path = $attach_dir . '/attachment-' . $managed_id . '-' . $attachment_uuid;
            dbg("=[_post]= : final data_path for attachment is $data_path");
        }
        else {
        ...
        if ( $self->check_write_access( $request->uri->path ) ) {    # this is a placeholder for ACLs, not a permissions check. this also assumes $data_path is given same access as request path
            ...
            my $orig_umask = umask(0077);
            my $tmp_path   = $data_path . _get_rand() . ".tmp";
            if ( open my $fh, '>', $tmp_path ) {
                dbg("=[_post]= : created temp file at $tmp_path");
                ...
                #             }
                close $fh;
                if ( _rename( $tmp_path, $data_path ) ) {
                    dbg("=[_post]= : Content written to ([$data_path])");
                }
                else {
                    dbg("=[_post]= : Could not rename ([$tmp_path]) to ([$data_path]) : $!");
                    $response->code('500');
                ...
                if ( $post_dispo_type eq 'attachment' && $filename ) {    # HB-7218 - small - check if we have action=attachment-add or action=attachment-update&managed-id=97S , etc in the $original_query_string
                    ...
                    my $request_url = $self->{'request_info'}{'uri_decoded_safe'};
                    dbg("=[_post]= : need to take $request_url and chop off the ics ?");
                    $request_url =~ s/^(.+)\.ics$/$1\.cpd/;
                    my $url = 'https://' . $request->{'_headers'}{'host'} . Cpanel::Encoder::URI::uri_encode_dirstr( $request_url . '/attachment-' . $managed_id . '-' . $attachment_uuid );
                    $self->_modify_event( 'add', 'attachment', $path, $data_path, $filename, $post_file_type, $total_bytes, $managed_id, $url );
...
sub _modify_event {    ##no critic(Subroutines::ProhibitManyArgs)
    my ( $self, $action, $action_type, $path, $data_path, $post_dispo_filename, $post_file_type, $total_bytes, $managed_id, $url ) = @_;
    logfunc();
    dbg("=[_modify_event]= : $action,$action_type,$path,$data_path,$post_dispo_filename,$post_file_type,$total_bytes,$managed_id,$url");
    if ( $action eq 'add' ) {
        if ( $action_type eq 'attachment' ) {
            ...
            if ( open( my $dav_fh, '<', $path ) ) {
                ...
                # Write it back out
                my $tmp_path = $path . _get_rand() . ".tmp";
                dbg("=[_modify_event]= : using temp path of $tmp_path");
                my $orig_umask = umask(0077);
...
sub check_if_over_upload_limit {
    ...
    logfunc();
    if ( !defined($bytes) ) {
        dbg("_[check_against_upload_limit]_ : ![bytes passed to function is undefined, which should not happen, logging for visbility]!");
        return 0;
    }
    my $upload_limit_bytes = $self->get_upload_limit();    # Use disk math for 2MB in bytes
    if ( $bytes > $upload_limit_bytes ) {
        dbg("_[check_against_upload_limit]_ : number of bytes ($bytes) being uploaded is greater than our limit, $upload_limit_bytes, sending 413 response.");
        $response->code(413);
...
sub remove_freebusy_data {
    ...
    my $privs_obj = Cpanel::DAV::CaldavCarddav::_drop_privs_if_needed($user);
    my $filelock  = Cpanel::SafeFile::safeopen( my $unused_lock_fh, '>', $fb_full_path . '.modify_lock' );

    if ( !$filelock ) {
        dbg("Could not get lock on $fb_full_path for writing : $!");
    }
    else {
        ...
        Cpanel::SafeFile::safeclose( $unused_lock_fh, $filelock );
        dbg("=[remove_freebusy_data]= : Modified Free-Busy data in $fb_full_path");
    }
    return;
}
...
sub modify_freebusy_data {
    ...
    # Lock the freebusy file before we load it, and unlock after it's saved.
    my $privs_obj = Cpanel::DAV::CaldavCarddav::_drop_privs_if_needed($user);
    my $filelock  = Cpanel::SafeFile::safeopen( my $unused_lock_fh, '>', $fb_full_path . '.modify_lock' );
    ...
    if ( !$filelock ) {
        ...
        save_freebusy_data( $fb_full_path, $user, $fb_data_hr );
        Cpanel::SafeFile::safeclose( $unused_lock_fh, $filelock );
        dbg("Modified Free-Busy data in $fb_full_path");
    }
    return;
}
...
sub load_freebusy_data {
    ...
    if ( !-f $fb_full_path || $fb_full_path !~ m/\.freebusy\.json$/ ) {
        dbg("([load_freebusy_data]) : no such file $fb_full_path");
        return {};
    }
    local $@;
    my $fb_data_hr = eval { Cpanel::JSON::LoadFile($fb_full_path) };
    if ($@) {
        my $msg    = "Failed to open $fb_full_path : $@";
        my @caller = caller();
        if ( $caller[0] eq 'Cpanel::DAV::CaldavCarddav' ) {    # This lets scripts calling from outside of this module not get caught up in needing a dbg() handler
    ...
    my $file_version = delete $fb_data_hr->{'__cp_fb_vers'};
    if ( length $file_version && $file_version =~ m/^\d+/ ) {
        dbg("([load_freebusy_data]) : Free-Busy file version in ([$fb_full_path]) : $file_version");
        return $fb_data_hr;
    }
    else {
        dbg("([load_freebusy_data]) : Could not find file version in ([$fb_full_path])");
        return undef;
    }
}
...
sub clean_attachments_from_raw_ics {
    ...
    my $ics_dir_name = $ics_filename;
    $ics_dir_name =~ s/^(.+)\.ics$/$1/;

    # We limit the files that can be unlinked to the collection the .ics is in matching $ics_filename-attachment-*
    # Note that this is the "old" way, before HB-7707, which addresses issues with long filenames
    ...
    # We now also limit the files that can be unlinked to the collection the .ics is in matching $ics_dir_name/attachment-*
    if ( opendir( my $ics_dir_fh, $col_dir . '/' . $ics_dir_name ) ) {
        while ( my $file = readdir($ics_dir_fh) ) {
            if ( $file =~ m/^attachment\-+/ ) {
                push( @attachments_on_disk, $ics_dir_name . '/' . $file );
            }
        }
        closedir($ics_dir_fh);
    }
    dbg( "_[clean_attachments_from_raw_ics]_ : $col_dir/$ics_dir_name/ and $ics_filename ", \@attachments_on_disk );
...
sub get_freebusy_data_from_parsed_ics {
    ...
    foreach my $event_ar ( @{$events_info_ar} ) {
        ...
        if ( length( $event_ar->{'dtstart'}{'params'}{'tzid'} ) ) {
            dbg("_[get_freebusy_data_from_parsed_ics]_ : about to call strptime on $event_ar->{'dtstart'}{'params'}{'tzid'}");
        }
        else {
            dbg("_[get_freebusy_data_from_parsed_ics]_ : about to call strptime on empty value, as we could not parse the dtstart tzid from event file");
        }
...
sub _update_partstat_for_attendee {    ##no critic(Subroutines::ProhibitExcessComplexity)
    ...
    if ( !length $final_destination ) {
        dbg("_[_update_partstat_for_attendee]_ : readlink on $symlinked_path returned nothing");
        return 8;
    }
...
sub _customize_transp_or_valarm {
    my ( $self, $dest_principal, $attendee_event_upload, $raw_ics_data_ref ) = @_;
    my $orig_filename = $attendee_event_upload =~ s/\.tmp$//r;

    my ( $transp_line, $valarm_section ) = _extract_transp_and_valarm($raw_ics_data_ref);

    dbg("=[_customize_transp_or_valarm]= : Checking whether changes are needed ...");

    my $orig_info = _load_regular_transp_and_valarm($orig_filename);
    if ( $orig_info->{transp_line} eq $transp_line and $orig_info->{valarm_section} eq $valarm_section ) {
        dbg("=[_customize_transp_or_valarm]= : provided TRANSP '$transp_line' and VALARM '$valarm_section' in $attendee_event_upload are identical to existing ones from $orig_filename, so no need to add overrides");
        return;
    }

    my ( $overrides_dir, $overrides_file ) = _overrides_dir_and_file($orig_filename);

    if ( !-d $overrides_dir ) {
        mkdir $overrides_dir, 0700;
    }

    dbg("=[_customize_transp_or_valarm]= : saving customized TRANSP and/or VALARM for $orig_filename");
    require Cpanel::JSON;

    Cpanel::JSON::DumpFile(
        $overrides_file,
        {
            transp_line    => $transp_line,
            valarm_section => $valarm_section,
        }
    );
...
sub _load_custom_transp_or_valarm {
    my ( $self, $symlinked_path, $raw_ics_data_ref ) = @_;

    require Cpanel::JSON;

    my $orig_filename = $symlinked_path =~ s/\.tmp$//r;
    my ( $overrides_dir, $overrides_file ) = _overrides_dir_and_file($orig_filename);

    if ( !-f $overrides_file ) {
        dbg("=[_load_custom_transp_or_valarm]= : no customizations found for $symlinked_path");
        return;
    }

    dbg("=[_load_custom_transp_or_valarm]= : loading customizations for $symlinked_path");
    my $data;
    eval {
        $data = Cpanel::JSON::LoadFile($overrides_file);
        die "missing transp_line\n"    if !$data->{transp_line};
        die "missing valarm_section\n" if !$data->{valarm_section};
    };
    if ( my $exception = $@ ) {
        dbg("=[_load_custom_transp_or_valarm]= : $overrides_file appears to be corrupted: $exception");
        return;
    }

    my $new_ics_data = $$raw_ics_data_ref;

    $new_ics_data =~ s/ ^ TRANSP:\S+? \s? $                        /$data->{transp_line}/gmx;
    $new_ics_data =~ s/ ^ BEGIN:VALARM \s? $ .* ^ END:VALARM \s? $ //gmxs;
    $new_ics_data =~ s/ ( ^ END:VEVENT )                           /$data->{valarm_section}\n$1/gmx;

    return \$new_ics_data;
}

sub _load_regular_transp_and_valarm {
    my ($orig_ics_file) = @_;

    open my $fh, '<', $orig_ics_file or return;
    my $content = do {
        local $/ = undef;
        readline $fh;
    };
    ...
    my ( $transp_line, $valarm_section ) = _extract_transp_and_valarm( \$content );
...
sub _extract_transp_and_valarm {
    my ($ics_ref) = @_;

    my ($transp_line)    = $$ics_ref =~ (m/ ^ ( TRANSP:\S+? \s? ) $ /mx);
    my ($valarm_section) = $$ics_ref =~ (m/ ( ^ BEGIN:VALARM \s? $ .* ^ END:VALARM \s? $ ) /mxs);

    s/\r//g for $transp_line, $valarm_section;

    return ( $transp_line, $valarm_section );
}

sub _overrides_dir_and_file {
    my ($orig) = @_;

    my @pieces   = split '/', $orig;
    my $filename = pop(@pieces) . '.json';
    $pieces[-1] = 'overrides';
    my $dir = join( '/', @pieces );
...
sub _is_path_too_long {
    my ($path) = @_;
    logfunc();

    # Split path into segments, if any segment is > 255 chars, reject it by returning 1
    my @parts = split( '/', $path );
    foreach my $segment (@parts) {
        if ( length $segment > 255 ) {
            dbg("_[_put]_ : ([$path]) has a segment that is more than 255 characters ([$segment]), denying.");
            return 1;
        }
    }
    return 0;
...
sub _put {    ## no critic qw(ProhibitExcessComplexity)
    ...
    if ( $self->check_write_access( scalar($path) ) ) {
        ...
        my ( $is_symlink, $is_attendee_update ) = ( 0, 0 );
        if ( -l $path ) {
            $is_symlink = 1;
            ...
            if ( not length( $proxy_config_hr->{$principal}{ $self->{'auth_user'} } ) or $proxy_config_hr->{$principal}{ $self->{'auth_user'} } ne 'calendar-proxy-write' ) {
                dbg("_[_put]_ : Currently authenticated user $self->{'auth_user'} is granted permission to read from ([$path]) via proxy config");
                $path               = $path . '.tmp';
                $is_attendee_update = 1;
            }
            else {
                dbg("_[_put]_ : Despite being a symlink, not calling this an attendee update because of the value of \$proxy_config_hr->{$principal}{ $self->{'auth_user'} }: '$proxy_config_hr->{$principal}{ $self->{'auth_user'} }'");
            }
        }
        my $tmp_path = $path . _get_rand() . '.tmp';
        if ( open my $fh, '>', $tmp_path ) {
            dbg("_[_put]_ : created file at ([$tmp_path])");
            $response->code(201);
            $response->message("CREATED");
            ...
            my $is_ics = 0;
            my $is_vcf = 0;
            if ( $path =~ m/\.ics(\.tmp)?$/i ) {    # These regexes should be against the final destination path, not tmp_path
                $is_ics = 1;
            }
            ...
            if ( _rename( $tmp_path, $path ) ) {
                dbg("_[_put]_ : wrote file to ([$path])");
            }
            else {
                dbg("_[_put]_ : Could not rename ([$tmp_path]) to ([$path]) : $!");
                $response->code('500');
                $response->message("Failed to rename file");
                goto QUICKEXIT;
            }
            ...
            if ($is_attendee_update) {
                ...
                # as per https://github.com/apple/ccs-calendarserver/blob/master/doc/Extensions/caldav-sharing.txt #5.5.4, we need to store
                # TRANSP and VALARM lines on a per-user basis, so one person's VALARM does not overwrite any other users' setting.
                # An attendee with the symlinked event can customize TRANSP and/or VALARM in an overrides file without editing the original ics.

                $self->_customize_transp_or_valarm( $self->{'auth_user'}, $path, \$original_vcard_data );

            }
            else {
                dbg("_[_put]_ : This is NOT an attendee update.");
                ...
                if ($is_symlink) {
                    dbg("_[_put]_ : not attempting to send any invites for this event because auth'd user is one of the invited attendees");
                }
                elsif (@attendees) {
                    ...
                    if ( $err_code == 5 ) {
                        ...
                        # What was expected to be a symlink was not, or readlink failed for another reason
                        $response->code(500);
                        $response->message("Server could not locate expected content");
                        return;
                    }
                }
            }
...
sub _delete {
    ...
    # https://datatracker.ietf.org/doc/html/rfc8607#section-3.9 says that users can not directly call DELETE on attachments. You have to delete it by omission
    ...
    if ( $path =~ m/\/.+\-attachment\-.+/ || $path =~ m/\/.+\/attachment\-.+/ ) {
    ...
    dbg("_[_delete]_ : about to find all related files like ics and attachments");
    ...
    if ( -f $path ) {
        ...
        my $ics_dir_name = $filename;
        if ( $ics_dir_name =~ m/(.+)\.ics$/ ) {
            $ics_dir_name = $1 . '.cpd';
        }

        dbg("_[_delete]_ : ([$path]) is a file, ics_dir_name is ([$ics_dir_name]) and dirpath is ([$dirpath])");

        @parts = File::Find::Rule->any(
            File::Find::Rule->name($filename),
            File::Find::Rule->name( $filename . '-attachment-*' ),
            File::Find::Rule->name($ics_dir_name),
        )->in($dirpath);
    ...
    foreach my $path_to_rm (@allowed_to_delete) {
        ...
        if ( $self->check_write_access( $self->_parentdir($path_to_rm) ) ) {
            ...
            if ( -f $path_to_rm ) {
                ...
                # HB-7213 - large
                #      - need to consider if we even want to allow this ?
    ...
    if ( $parsed_request_hr->{'reqinfo'} eq 'set' ) {
        ...
        foreach my $prop ( @{ $parsed_request_hr->{'reqprops'} } ) {
            ...
            if ( grep( /\Q$propname\E/, @Cpanel::DAV::Metadata::metadata_props ) ) {
                ...
                my $tmp_avail_path = $avail_path . _get_rand() . '.tmp';
...
sub _request_wrapper {    ##no critic(Subroutines::ProhibitExcessComplexity)
    ...
    if ( defined $depth and $depth == 1 and not( $reqtype eq 'calendar-multiget' and @hrefs ) and -d $path ) {
        ...
        @paths = grep { !/(\.metadata$|\.sharing$|\.freebusy\.json|\.availability|\.cpd$|\-attachment\-|\.tmp|calendar\-proxy\-read|calendar\-proxy\-write$)/ } @paths;    # the proxy paths are virtual, will they be here ?
                                                                                                                                                                           # Note that .cpd paths self identify as "cpanel directories", without having to stat for -d
...
sub _load_properties_lookup_table {    ##no critic(Subroutines::ProhibitExcessComplexity)
    ...
    $lt{'urn:ietf:params:xml:ns:caldav'}{'calendar-data'}{'cr'}     = sub {
        my ( $self, $doc, $prop ) = (@_);
        my $path_on_disk = $self->{'current_path_info'}{'req_path'};
        dbg("_[prop]_ [-[calendar-data]-] : asking get_data_from_file on ([$path_on_disk]) for data");
        my $caldata = $self->get_data_from_file($path_on_disk);

        if ( -l $path_on_disk && $path_on_disk =~ /\.ics/ && ( my $customized_ics_ref = $self->_load_custom_transp_or_valarm( $path_on_disk, \$caldata ) ) ) {
            dbg("_[prop]_ [-[calendar-data]-] : applying customized transp and/or valarm for $path_on_disk");
            $caldata = $$customized_ics_ref;
        }
        else {
            dbg("_[prop]_ [-[calendar-data]-] : no need to apply custom transp and/or valarm for $path_on_disk");
        }
