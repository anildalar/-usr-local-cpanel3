...
True if the DNS server currently supports C<ALIAS> records.

=item aname

True if the DNS server currently supports C<ANAME> records.

Currently this is always false. It is included for completeness and future proofing.

If ANAME support is ever possible in pdns and added to cPanel/WHM (Would need added to message that MOON-1939 updated, as well as the UI changes like MOON-1940 and MOON-1939) this will change.
...
#### IF THIS CHANGES ALSO UPDATE Whostmgr::API::1::DNS::is_alias_available() TO MATCH! ##
sub is_alias_available ( $args, $result, @ ) {    # third arg seems wonky … but … errors

    # Users can see this file
    if ( !-e "/etc/pdns/danger-danger-danger-server-owners-understand-the-severe-risk-and-accept-responsibility-for-any-consequences-of-using-experimental-alias-records" ) {
        return $result->data( { alias => 0, aname => 0, resolver => undef } );
    }

    if ( !-x '/usr/bin/pdns_control' ) {    # under bind, or no DNS server this will not exist
        return $result->data( { alias => 0, aname => 0, resolver => undef } );
    }

    require Cpanel::AdminBin::Call;
    my %pdns = %{ Cpanel::AdminBin::Call::call( 'Cpanel', 'zone', 'GET_PDNS_ALIAS_CONF' ) };

    # expand-alias and expand-alias= dump out as expand-alias=. That and, say expand-alias=" " all still mean yes
    my $expand = 0;
    if ( exists $pdns{"expand-alias"} && $pdns{"expand-alias"} ne "off" && $pdns{"expand-alias"} ne "no" ) {
        $expand = 1;
    }

    # resolver, if given, must have a value or pdns won’t start
    my $resolver;
    if ( exists $pdns{resolver} && $pdns{resolver} ne "off" && $pdns{resolver} ne "no" ) {
        $resolver = $pdns{resolver};
    }

    if ( $expand && $resolver ) {
        return $result->data( { alias => 1, aname => 0, resolver => $pdns{resolver} } );
    }

    return $result->data( { alias => 0, aname => 0, resolver => $pdns{resolver} } );
}
...
sub _authorize_record_type ( $result, $type, $allowed_hr ) {
    if ( $type =~ m/ALIAS/ ) {
        ...
        my $res = Cpanel::Result->new();
        is_alias_available( {}, $res, undef );

        if ( !$res->{data}{alias} ) {
            delete( %$allowed_hr{$type} );
        }
    }
