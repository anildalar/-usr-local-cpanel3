...
sub GET_PDNS_ALIAS_CONF {
    my ($self) = @_;

    # caller should have already returned if !-x /usr/bin/pdns_control
    # worst case is a warning, which we want because the caller is doing it wrong
    # also unlikely to be called outside of Cpanel::API::DNS::is_alias_available
    # Which is why its not documented
    return { map { chomp; split /=/, $_ } `/usr/bin/pdns_control current-config | grep -E "^expand-alias|^resolver"` };
}
