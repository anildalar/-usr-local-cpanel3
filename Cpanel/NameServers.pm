...
=head2 compare_nameservers_arefs ($ns_ar1, $ns_ar2)

Returns a boolean indicating whether the ArrayRefs of nameservers passed in are the same.

=cut

sub compare_nameservers_arefs ( $ns_ar1, $ns_ar2 ) {
    my $ns1_str = join( '|', sort map { lc } @$ns_ar1 );
    my $ns2_str = join( '|', sort map { lc } @$ns_ar2 );

    return $ns1_str eq $ns2_str ? 1 : 0;
}
