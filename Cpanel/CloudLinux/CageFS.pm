...
sub is_enabled () {
    my $cagefsctl = Cpanel::Binaries::path('cagefsctl');
...
sub enabled_users () {
    ...
    my $result_cr = Cpanel::CachedCommand::cachedcommand_no_errors( 'binary' => Cpanel::Binaries::path('cagefsctl'), 'args' => CAGEFSCTL_ENABLED_USERS_ARGS(), 'mtime' => _latest_users_timestamp(), 'ttl' => MAX_CACHE_AGE() );
...
sub force_cagefs_update {
    ...
    return _run_cagefsct( 'force_cagefs_update' => ['--force-update'] );
}

sub cagefs_remount_all() {
    return 0 unless Cpanel::CloudLinux::CageFS::is_enabled();

    return _run_cagefsct( 'remount_all' => ['-M'] );
}

sub _run_cagefsct ( $name, $args ) {
    $name //= '';
    $args //= [];

    return 0 unless Cpanel::CloudLinux::CageFS::is_enabled();

    my $bin = Cpanel::Binaries::path('cagefsctl');
    ...
    if ( $run->CHILD_ERROR() ) {
        ...
        $logger->info("Cpanel::CloudLinux::CageFS::$name -- Failed to run $bin @$args: $out");
        return;
    }
