...
sub clean ($self) {
    return $self->syscmd( sub { return undef }, "clean" );
}

sub cache ($self) {

    # syscmd() dies on failure
    $self->syscmd( sub { return undef }, "update" );    # resynchronize the package index files

    # no good way to local() an attribute so here is a gross hack
    local $self->{syscmd_binary} = '/usr/bin/apt-cache';
    $self->syscmd( sub { return undef }, "gencaches" );    # creates APT's package cache
