...
sub find_mount {
    ...
    if ( ref $partitions eq 'HASH' ) {    #AKA Cpanel::Filesys::Info::all_filesystem_info, Cpanel::DiskLib::get_disk_used_percentage
        if ( values %{$partitions} && defined( ( values %{$partitions} )[0]->{'used'} ) ) {    # Cpanel::DiskLib::get_disk_used_percentage
            ...
            foreach my $mount ( sort { length $b <=> length $a } keys %{$partitions} ) {    # Cpanel::Filesys::Info::all_filesystem_info
