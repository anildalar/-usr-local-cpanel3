...
sub generate_config_file ($self) {    ## no critic(ProhibitExcessComplexity)
  ...
  RBL_TEMPLATE_LOOP:
    ...
    if ( $SETTINGS{'arc_signing'} ) {

        # The arc_sign line is always existing in exim.conf, but inside an .ifdef block, so we can enable it just by setting the option here
        push @CONFIG_OPTS, 'ARCSIGNING = yes';

    }

    if ( $ACLS{'dkim_bl'} ) {
        # The arc_sign line is always existing in exim.conf, but inside an .ifdef block, so we can enable it just by setting the option here
        push @CONFIG_OPTS, 'DKIMREJECT = yes';
    }
