...
sub installed_cpanel_obsoletes () {
    ...
    # Filter out only known packages provided by check_cpanel_pkgs. As of 98, ALL cpanel-rpms should begin with cpanel-.
...
=head2 lock_for_external_install ( $logger = undef )
...
If you pass a logger object as the first argument it will be used,
otherwise a new Cpanel::Logger instance is created.

=cut

sub lock_for_external_install ( $logger = undef ) {
    $logger //= _logger();
