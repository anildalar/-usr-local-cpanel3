...
sub disable_service {
    ...
    my $skip_if_missing = delete $opts->{'skip_if_missing'};
    ...
    if ( !$unit_state ) {
        ...
        return if $skip_if_missing;
        die "The '$service' service does not exist.";
    }
...
sub enable_service {
    ...
    my $skip_if_missing = delete $opts->{'skip_if_missing'};
    my $start_service   = delete $opts->{'start_service'} // 1;
    ...
    if ( $was_enabled || $force ) {
        $self->regular_logmsg(
            sprintf(
                "%s the '%s' service.%s",
                ( $start_service ? 'Enabling and starting' : 'Enabling' ),
                $service,
                ( ( $force && !$was_enabled ) ? ' (FORCED)' : '' ),
            )
        );
        if ($start_service) {
