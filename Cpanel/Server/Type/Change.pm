...
sub _enable_profile_services_and_roles {
    ...
    # adding hooks for enabling services
    my %hooks_data = (
        'services'      => \@enable_services,
        'changed_roles' => $opts{changed_roles},
        'action'        => 'enable',
    );

    _add_hooks(%hooks_data);
...
sub _disable_profile_services_and_roles {
    ...
    # adding hooks for disabling services
    my %hooks_data = (
...
sub _add_hooks {
    my %opts  = @_;
    my $type  = $opts{'action'};
    my $event = "Server::Type::Change::$type";
    eval {
    ...
    if ( my $err = $@ ) {
        logger()->warn( "Hooks system failed to run for '$type' services. " . Cpanel::Exception::get_string($err) );
        return 0;
    }
